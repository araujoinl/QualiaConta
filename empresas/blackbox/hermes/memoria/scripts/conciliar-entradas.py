...
Cruza las entradas de dinero del banco (Supabase/openbanking) contra los
registros de ingreso de ADM Cloud, para identificar qué entradas del banco
no están registradas en ADM y viceversa.
...
OUTPUT:
    - stdout: resumen en texto
    - Excel:  ./Conciliacion_Entradas_<mes>.xlsx
    - JSON:   ./conciliacion_<mes>.json  (para integración)
...
CUENTAS_EXCLUIDAS = [
    "11122010025676",  # Ahorro General — Impact Logistics
]
...
# Tolerancias de cruce
MONTO_EXACTO_DIFF = 0.50       # DOP
MONTO_CERCANO_PCT = 0.005      # 0.5%
MONTO_FUZZY_PCT   = 0.01       # 1%
FECHA_MAX_EXACTO  = 10         # días
FECHA_MAX_CERCANO = 7          # días
FECHA_MAX_FUZZY   = 5          # días

# Sufijos legales a quitar en normalización de nombres
SUFFIXES = re.compile(
...
def names_match(bank_desc, adm_name):
    """Compara nombres normalizados del banco y de ADM."""
...
def load_bank_txs(start_date, end_date):
    """Lee créditos (monto > 0) de Supabase para cuentas Blackbox."""
    import subprocess
...
def admcloud_get_all(resource):
    """Descarga todas las páginas de un recurso de ADM Cloud."""
    ...
    while True:
        ...
        # Unwrap (la API envuelve de varias formas)
...
def load_adm_txs(start_date, end_date):
    """Carga los 3 tipos de entradas de ADM Cloud en el período."""
    adm = []

    # 1. CashInvoices (Ventas de Contado)
    ...
    # 2. CashReceipts (Recibos de Cuentas por Cobrar)
    ...
    # 3. BankBankTransfers (Transferencias entre cuentas — lado entrada)
...
def match_rounds(bank, adm):
    ...
    # --- Rondas tarjetas (monto original, sin nombre) ---
...
def generate_excel(matched, unmatched_bank, unmatched_adm, output_path):
    ...
    ws = wb.active
    ws.title = "Resumen"
    ws["A1"] = "Entradas de Dinero: Banco vs ADM Cloud"
    ws["A1"].font = tf
    ...
    r = 5
    ws.cell(row=r, column=1, value="RESUMEN").font = sf
    r += 1
    ...
    for label, cnt, monto in stats:
        if label:
            ...
            if "CONCILIADOS" in label:
                ...
                for col in range(1, 4):
                    ws.cell(row=r, column=col).fill = yfill
            elif "ADM" in label and "ADM Cloud" not in label:
                for col in range(1, 4):
                    ws.cell(row=r, column=col).fill = ofill
        r += 1
    ...
    for i, (tx, is_m, bk) in enumerate(all_adm, 2):
        vals = [
            i - 1, tx["fecha"], tx.get("doc_id", ""), tx.get("tipo", ""),
...
def main():
    # Parsear argumentos: mes o rango
    if len(sys.argv) >= 3:
